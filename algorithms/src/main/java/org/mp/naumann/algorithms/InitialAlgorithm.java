 ...
 *
 * An initial algorithm is an algorithm that is executed exactly once
 * at the time when the system is setup.
 * It <b>only</b> works on the pre-existing dataset and is <u>not</u> executed again for or before every batch.
 *
 * During its course, the initial algorithm is supposed to create an intermediate datastructure, which can be reused by the
 * {@link InitialAlgorithm}. This datastructure stores any additional information that is necessary for further processing,
 * but information that is not necessarily actually wanted output of the algorithm.
 *
