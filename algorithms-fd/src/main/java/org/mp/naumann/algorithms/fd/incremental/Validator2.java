...
public class Validator2 {
	...
	public Validator2(FDSet negCover, FDTree posCover, int[][] compressedRecords, List<PositionListIndex> plis, float efficiencyThreshold, boolean parallel, MemoryGuardian memoryGuardian) {
	...
	private class ValidationTask implements Callable<ValidationResult> {
		...
		public ValidationResult call() throws Exception {
			...
			if (Validator2.this.level == 0) {
				// Check if rhs is unique
				for (int rhsAttr = rhs.nextSetBit(0); rhsAttr >= 0; rhsAttr = rhs.nextSetBit(rhsAttr + 1)) {
					if (!Validator2.this.plis.get(rhsAttr).isConstant(Validator2.this.numRecords)) {
			...
			else if (Validator2.this.level == 1) {
				// Check if lhs from plis refines rhs
				int lhsAttribute = lhs.nextSetBit(0);
				for (int rhsAttr = rhs.nextSetBit(0); rhsAttr >= 0; rhsAttr = rhs.nextSetBit(rhsAttr + 1)) {
					if (!Validator2.this.plis.get(lhsAttribute).refines(Validator2.this.compressedRecords, rhsAttr)) {
						element.removeFd(rhsAttr);
						result.invalidFDs.add(new FD(lhs, rhsAttr));
					}
					result.intersections++;
				...
				OpenBitSet validRhs = Validator2.this.plis.get(firstLhsAttr).refines(Validator2.this.compressedRecords, lhs, rhs, result.comparisonSuggestions);
	...
	public List<IntegerPair> validatePositiveCover() throws AlgorithmExecutionException {
		...
		List<FDTreeElementLhsPair> lvl = null;
		if (this.level == 0) {
			lvl = new ArrayList<>();
			lvl.add(new FDTreeElementLhsPair(this.posCover, new OpenBitSet(numAttributes)));
		}
		else {
			lvl = this.posCover.getLevel(this.level);
		}

		List<FDTreeElementLhsPair> currentLevel = pruneLevel(lvl);
		...
		while (!currentLevel.isEmpty()) {
			...
			currentLevel = pruneLevel(nextLevel);
			this.level++;
			int numInvalidFds = validationResult.invalidFDs.size();
			int numValidFds = validationResult.validations - numInvalidFds;
